...
function createDetector(onVerdict) {
  ...
  function frame(buf, off) {
    ...
    // Threshold rides the measured noise floor. The absolute floor exists only
    // to ignore idle-channel hiss — real phone voices can be VERY quiet (the
    // first backtest against recorded calls missed 35 real humans at 700).
    const speaking = rms > Math.max(noise * 4, 260) && !beep;
    // Learn the floor only in true silence: the dips between words are not
    // background noise, and learning from them poisons the threshold mid-greeting.
    if (!speaking && utterStartMs < 0) noise = noise * 0.97 + rms * 0.03;
